...
sudo mkdir -p /opt/tetrapod/{authelia/{data,redis,secrets},caddy/{data,config},jellyfin/config,jellyfin/cache,qbittorrent,prowlarr,sonarr,radarr,bazarr,pinchflat,suwayomi} /opt/tetrapod/{zipline/{uploads,public,db},shlink,media-reconcile}
...
sudo chown -R 1000:1000 /opt/tetrapod/{jellyfin,qbittorrent,prowlarr,sonarr,radarr,bazarr,pinchflat,suwayomi} /srv/media/library/manga
...
if opa read 'op://Agents/TETRAPOD_PUBLIC_SERVICES/password' >/dev/null 2>&1; then
  (cd "$HERE" && public-services-provision) || echo "WARN: public service provisioning failed"
  sudo docker compose -f "$HERE/docker-compose.yml" up -d caddy
else
